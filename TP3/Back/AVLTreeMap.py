...
class AVLTreeMap( TreeMap ):
    ...
    def find_close(self, k):
        ...
        greater_than = None
        less_than = None
        if self.find_gt(k) is not None:
            greater_than = self.find_gt(k)[0]
        if self.find_lt(k) is not None:
            less_than = self.find_lt(k)[0]

        if greater_than is None:
            return less_than
        elif less_than is None:
            return greater_than
        else:
            if math.fabs(less_than-k) > math.fabs(less_than-k):
                return less_than
            elif math.fabs(less_than-k) == math.fabs(less_than-k):
                return greater_than
            else:
